...
fresh-nocache:  ## Destroy & recreate all uing dev containers.
	make stop
	make destroy
	make build-nocache
	make start
...
fresh-prod-nocache: ## Destroy & recreate all using prod containers.
	make stop-prod
	make destroy-prod
	make build-prod-nocache
	make start-prod

ssh: ## SSH into PHP container
	docker exec -it ${CONTAINER_PHP} sh

shell: ## SSH into PHP container
